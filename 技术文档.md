...
```text
https://app.clouduse01.com/monitor/
```
...
| 公网 IP | `3.121.98.176` |
| SSH 用户 | `ec2-user` |
| SSH Key | `aws-APP-20250916.pem`，仅保存在本地，已加入 `.gitignore` |
| 系统 | Amazon Linux 2023 |
| 应用目录 | `/home/ec2-user/qv-monitor` |
| 外层 Nginx 实际加载文件 | `/etc/nginx/conf.d/app.conf` |
| 外层 Nginx 编辑副本 | `/home/ec2-user/app.conf` |
| Docker Compose | `/usr/local/bin/docker-compose` |
| Docker 内 Nginx 端口 | `127.0.0.1:8081 -> monitor-ui:80` |

当前线上容器：

```text
monitor-ui  nginx:stable-alpine
...
```mermaid
flowchart LR
  Browser["浏览器 / QV 云"] --> PublicNginx["系统 Nginx :443"]
  PublicNginx -->|/monitor/ 和 /api/*| DockerNginx["monitor-ui :8081"]
  DockerNginx --> PWA["dist 静态 PWA"]
  DockerNginx --> MediaMTX["MediaMTX :8889 / :9997"]
```
...
```mermaid
flowchart LR
  QV["QV 云报警推送"] -->|POST https://app.clouduse01.com/api/events/push| PublicNginx["系统 Nginx :443"]
  PublicNginx -->|/api/events/| DockerNginx["monitor-ui :8081"]
  DockerNginx -->|/api/events/| EventAPI["event-api :3001"]
  EventAPI --> SQLite["/home/ec2-user/qv-monitor/event-data/events.sqlite"]
  PWA["/monitor/alarms"] -->|GET /api/events| EventAPI
```
...
第一阶段暂不启用 token 校验，便于 Postman 和对方服务器联调。服务端实现时仍应预留可选 token 配置能力：当 `EVENT_WEBHOOK_TOKEN` 为空时不校验；当该环境变量有值时，才校验 `X-Webhook-Token`。

### 7.2 部署脚本

仓库中的 `deploy.sh` 应使用当前生产服务器配置：
...
2. 通过 SSH 创建远端目录。
3. 备份远端当前 `dist`。
4. 上传 `dist`、`docker-compose.yml`、`nginx.conf`、`mediamtx.yml`。
5. 如果存在 `server/`，同时上传报警接收服务源码和 lockfile，不上传 `server/node_modules`。
6. 远端执行 `docker-compose up -d --build`。

注意：`deploy.sh` 不自动修改系统 Nginx 的实际加载文件 `/etc/nginx/conf.d/app.conf`。新增 `/api/events/` 反向代理后，需要先更新 `/home/ec2-user/app.conf`，再复制到 `/etc/nginx/conf.d/app.conf`，并执行 `sudo nginx -t && sudo systemctl reload nginx`。
...
生产环境中，`event-data` 将位于：

```text
/home/ec2-user/qv-monitor/event-data
```

该目录不得提交到 Git，当前已通过 `.gitignore` 忽略。
...
生产环境通过系统 Nginx 的 `/etc/nginx/conf.d/app.conf` 代理到 Docker Nginx。新增报警模块后，需要在该配置中增加：
...
| 项目 | 建议 |
| --- | --- |
| 认证 | 第一阶段暂不启用 token；服务端预留 `EVENT_WEBHOOK_TOKEN` 可选配置，后续启用时通过 `X-Webhook-Token` 校验 |
| Body 大小 | 限制为 `1mb` |
| 日志 | 使用 morgan 记录来源 IP、command、处理耗时、成功/失败状态码 |
| 去重 | 对 `alarmUniqueId` 或消息 ID 建唯一约束 |
| 错误处理 | 解析失败时返回非 0 code，并写错误日志 |
| 密钥 | 如果后续启用 token，只能通过服务器环境变量注入，不写死到前端 |
...
`EVENT_WEBHOOK_TOKEN` 规则：

```text
为空：不校验 token，第一阶段默认方式。
有值：要求请求 Header 携带 X-Webhook-Token，并与环境变量一致。
```

暂不建议使用 query token 作为生产方案，避免 token 出现在访问日志和浏览器历史中。
...
```bash
curl -X POST "http://localhost:3001/api/events/push" \
